...
export const exchangeCodeForToken = async (code, clientId, clientSecret) => {
  ...
  const { data } = await axios.post('https://github.com/login/oauth/access_token', body.toString(), {
    headers: {
...
export const analyzeRepository = async (accessToken, owner, repo) => {
  ...
  const configFiles = filePaths.filter((p) => /package\.json|requirements\.txt|Cargo\.toml|go\.mod|pom\.xml|dockerfile|docker-compose/i.test(p));
