...
export const generateAIResponse = async (systemPrompt, userPrompt) => {
  ...
  if (provider === 'openrouter') {
    try {
      ...
      if (msg.includes('user not found') || msg.includes('unauthorized') || msg.includes('401')) {
        const openaiApiKey = env.ai.openai?.apiKey;
        if (!openaiApiKey || openaiApiKey.includes('your_') || openaiApiKey.includes('your-')) {
          throw new Error(`OpenRouter API error: ${err.message}. No OpenAI fallback configured — set OPENAI_API_KEY in .env.local or switch AI_PROVIDER to openai`);
        }
